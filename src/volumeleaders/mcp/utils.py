...
def resolve_client(ctx: Context) -> VolumeLeadersClient:
    ...
    msg = "MCP lifespan context is missing VolumeLeaders client"
...
def one_week_ago_date_string() -> str:
    ...
    return (datetime.now(tz=UTC) - timedelta(weeks=1)).strftime("%Y-%m-%d")


def ninety_days_ago_date_string() -> str:
    """Return the UTC date 90 days ago in YYYY-MM-DD format."""
    return (datetime.now(tz=UTC) - timedelta(days=90)).strftime("%Y-%m-%d")


def one_year_ago_date_string() -> str:
    """Return the UTC date one year ago in YYYY-MM-DD format."""
    return (datetime.now(tz=UTC) - timedelta(days=365)).strftime("%Y-%m-%d")
...
def format_dollars(amount: float) -> str:
    ...
    """
    if amount >= _BILLION:
        return f"${amount / _BILLION:.1f}B"
    if amount >= _MILLION:
        return f"${amount / _MILLION:.1f}M"
    if amount >= _THOUSAND:
        return f"${amount / _THOUSAND:.0f}K"
    return f"${amount:.0f}"
