...
def _make_rows(
    ...
    """Build SectorBreakdown model rows from fixture."""
    return [
        SectorBreakdown.model_validate(row)
        for row in sample_sector_breakdown_response
    ]
...
def test_sector_flows_api_error_warning(
    ...
    result = sector_flows_tool(ctx=mcp_context)
    assert result["data"] == []
